...
import net.atos.kawwaportal.components.tree.DefaultTreeExpansionModel;
import net.atos.kawwaportal.components.tree.TreeExpansionModel;
import net.atos.kawwaportal.components.tree.TreeModel;
import net.atos.kawwaportal.components.tree.TreeNode;
 ...
 * expandable/collapsable/selectable nodes. The data that is displayed by the
 * component is provided as a {@link TreeModel}. A secondary model, the
 * {@link TreeExpansionModel}, is used to track which nodes have been expanded.
 * The optional {@link TreeSelectionModel} is used to track node selections (as
 * currently implemented, only leaf nodes may be selected).
 * <p/>
 * The Tree component uses special tricks to support recursive rendering of the
 * Tree as necessary.
 * 
 * @since 5.3
 */
...
@Import(stylesheet = { "classpath:/net/atos/kawwaportal/components/assets/tree/tree.css" })
public class Tree {

	@Inject
	@Symbol(value = JQuerySymbolConstants.SUPPRESS_PROTOTYPE)
	private boolean prototype;
	/**
	 * The model that drives the tree, determining top level nodes and making
	 * revealing the overall structure of the tree.
	 */
	@Parameter(required = true, autoconnect = true)
	private TreeModel model;

	/**
	 * Allows the container to specify additional CSS class names for the outer
	 * DIV element. The outer DIV always has the class name "t-tree-container";
	 * the additional class names are typically used to apply a specific size
	 * and width to the component.
	 */
	@Parameter(name = "class", defaultPrefix = BindingConstants.LITERAL)
	private String className;

	/**
	 * Optional parameter used to inform the container about what TreeNode is
	 * currently rendering; this is primarily used when the label parameter is
	 * bound.
	 */
	@Property
	@Parameter
	...
	/**
	 * Used to control the Tree's expansion model. By default, a persistent
	 * field inside the Tree component stores a
	 * {@link DefaultTreeExpansionModel}. This parameter may be bound when more
	 * control over the implementation of the expansion model, or how it is
	 * stored, is required.
	 */
	...
	/**
	 * Used to control the Tree's selections. When this parameter is bound, then
	 * the client-side Tree will track what is selected or not selected, and
	 * communicate this (via Ajax requests) up to the server, where it will be
	 * recorded into the model. On the client-side, the Tree component will add
	 * or remove the {@code t-selected-leaf-node-label} CSS class from
	 * {@code span.t-tree-label} for the node.
	 */
	@Parameter
	private TreeSelectionModel selectionModel;

	/**
	 * Optional parameter used to inform the container about the value of the
	 * currently rendering TreeNode; this is often preferable to the TreeNode,
	 * and like the node parameter, is primarily used when the label parameter
	 * it bound.
	 */
	@Parameter
	 ...
	 * tree node. This will be invoked after the {@link #value} parameter has
	 * been updated.
	 */
	@Property
	...
	public void setupRender() {
		String path = "/net/atos/kawwaportal/components/assets/tree/tree.js";
		if (prototype)
			path = "/net/atos/kawwaportal/components/assets/tree/tree-jquery.js";
		jss.importJavaScriptLibrary(source.getClasspathAsset(path));
	...
	private static RenderCommand RENDER_LABEL_SPAN = new RenderCommand() {
		public void render(MarkupWriter writer, RenderQueue queue) {
			...
			writer.element("a", "class", "t-tree-label", "href", "#");
		}
	};
	 ...
	 * Render commands to do the rest. May recursively render child nodes of the
	 * active node.
	 * 
	 * @param node
	 *            to render
	 ...
	 */
	private RenderCommand toRenderCommand(final TreeNode node,
			final boolean isLast) {
		return new RenderCommand() {
			public void render(MarkupWriter writer, RenderQueue queue) {
				// Inform the component's container about what value is being
				// rendered
				// (this may be necessary to generate the correct label for the
				// node).
				Tree.this.node = node;
				...
				if (hasChildren) {
					Link expandChildren = resources.createEventLink(
							"expandChildren", node.getId());
					Link markExpanded = resources.createEventLink(
							"markExpanded", node.getId());
					Link markCollapsed = resources.createEventLink(
							"markCollapsed", node.getId());
	...
	/**
	 * Renders an &lt;ul&gt; element and renders each node recursively inside
	 * the element.
	 */
	...
	Object onSelect(String nodeId,
			@RequestParameter("t:selected") boolean selected) {
		...
		if (selected) {
			...
			event = TreeConstants.NODE_SELECTED;
		} else {
			selectionModel.unselect(node);

			event = TreeConstants.NODE_UNSELECTED;
		}
	...
	/**
	 * Returns the actual {@link TreeExpansionModel} in use for this Tree
	 * component, as per the expansionModel parameter. This is often, but not
	 * always, the same as {@link #getDefaultTreeExpansionModel()}.
	 */
	 ...
	 * component, as per the {@link #selectionModel} parameter.
	 */
