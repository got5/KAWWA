Tapestry.Initializer.treeNode = function(spec) {
	var cfg = {
	...
	function successHandler(reply) {
		...
		Tapestry.loadScriptsInReply(response, function() {
			...
			var label = element.down("a.t-tree-label");
	...
	if (spec.selectURL) {
		...
		var label = $(spec.clientId).next("a.t-tree-label");
		...
		label.observe("click", function(event) {
			...
			// TODO: In the future, we may want to select children when a parent
			// is selected,
			// or vice-versa. There's a lot of use cases. These will be directed
			// from new methods
			// on the TreeSelectionModel interface and encoded into the
			// response. For now,
			// the response is empty and ignored.

			Tapestry.ajaxRequest(spec.selectURL, {
				parameters : {
					"t:selected" : selected
				}
			});
